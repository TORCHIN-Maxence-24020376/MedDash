/*
  EDT Grid (vanilla JS)
  - Affiche les cours dans .calendar-grid avec traits par heure + suivi temps réel
  - Parse un ICS (unique, déclaré en haut du fichier)
  - Pas de classes spécifiques par cours (utilise uniquement .cour existante)
...
*/
(function(){
    const START_HOUR = 7;      // heure de début (incluse)
    const END_HOUR   = 20;     // heure de fin (incluse visuelle, les events après ne s'affichent pas)
    ...
    // "today" : force toujours aujourd'hui (pratique pour tester le soir)
    // "tomorrow": force toujours demain
    const VIEW_MODE = "auto"; // <-- mets "auto" en prod
  
    
    // === Cible du jour à afficher : si >= 21h, on bascule sur le lendemain ===
    function getTargetDate(){
      ...
      } // if VIEW_MODE === "today" -> rien
      target.setHours(0,0,0,0);
      return target;
    }
    function shouldUpdateAt(){
      const h = new Date().getHours();
      return h < 21 ? 21 : 24; // 24 -> minuit (prochain jour)
    }
