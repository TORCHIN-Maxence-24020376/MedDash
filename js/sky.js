...
// Persist moon position across pages (localStorage)
...
function loadMoonState(){
  try{
    ...
    if(s && typeof s.angle==='number' && typeof s.t==='number'){
      const dt = Date.now() - s.t; // ms écoulées
      // Avance l'angle en fonction du temps passé pour continuité naturelle
      moon.angle = s.angle + moon.speed * dt;
      return;
    }
  }catch(e){}
  // fallback: départ hors écran
  resetMoon();
}
...
// Lune avec mouvement en arc de cercle
const moon = { angle: Math.PI, r: 56, rx: 0, ry: 0, cx: 0, cy: 0, speed: 0.0001, img: null }; // vitesse définie ici et uniquement ici
...
function updateOrbit(){
  // Responsive orbit + responsive moon size
  const w = canvas.width, h = canvas.height;
  const m = Math.min(w, h);
  moon.rx = w * 0.62;              // large arc horizontal
  moon.ry = h * 0.22;              // amplitude verticale
  moon.cx = w / 2;                 // centré en largeur
  moon.cy = h * 0.50;              // ~milieu de la page
  // Taille de la lune responsive (bornée)
...
loadMoonState();
requestAnimationFrame(frame);
// Sauvegarde périodique au cas où
