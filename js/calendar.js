(function () {
  ...
  // --- cache ICS (évite re-fetch à chaque nav)
  ...
  function makeHeader(container, weekStart, weekEnd) {
    ...
    const mkBtn = (iconPath, label, onClick) => {
      ...
      btn.type = "button";                         // ← évite submit/refresh
  ...
  async function loadAndRender(){
    ...
    if (isCurrentWeek && columns.length){
      ...
      // scroll au 1er cours de la semaine visible
  ...
  // Raccourcis clavier (sans interférer avec d’autres handlers)
